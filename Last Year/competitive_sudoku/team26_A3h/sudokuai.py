 ...
 #           state.current_player = 3 - current_player
        ...
        def monte_carlo_tree_search(root_state, iterations):
            ...
            for _ in range(iterations):
                ...
                result = minimax_rollout(leaf_node)
                # Backpropagation
                current = leaf_node
        ...
        def minimax_rollout(node:MCTSNode):
            """
            Implement minimax algorithm for the rollout policy
            
            @Param node: A MCTS node object representing the current node state.
            @Return eval_val_score_MC() : function to evaluate the score of the current game state
            """

            
            node = MCTSNode(game_state)
            _,best_move = minimax(node.state, 2, float('-inf'), float('inf'), True)
            
            if best_move: # if the best move proposed by minimax is True
                node.state = play_move(game_state, game_state.current_player, best_move)
            else:
                possible_moves = [
                    Move((i, j), value) for i in range(node.state.board.N) for j in range(node.state.board.N)
                    for value in range(1, node.state.board.N + 1)
                    if possible(i, j, value) and is_valid_move(node.state.board, (i, j), value)
                ]
                if possible_moves: # otherwise propose a random move, extracted from the possible legal moves
                    random_move = random.choice(possible_moves)
                    node.state = play_move(node.state, node.state.current_player, random_move)

            return eval_score_MC(node.state, node.state.current_player)
        ...
        move = random.choice(moves)  # Fallback random move
