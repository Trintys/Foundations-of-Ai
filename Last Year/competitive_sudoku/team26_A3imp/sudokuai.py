...
class SudokuAI(competitive_sudoku.sudokuai.SudokuAI):
    ...
    def compute_best_move(self, game_state: GameState) -> None:
        ...
        def minimax(state,allowed,heuristic,depth, alpha, beta, isMaximisingPlayer,map,counter):
            """
            ...
            """
            #check if gamestate has an already found solution
            #self.save()
            ...
            if isMaximisingPlayer:
                ...
                for child, move,score,allowed in children_and_move:
                    ...
                    if allowed==False:
                        continue
                    child_value, _ = minimax(child,allowed,False,depth - 1, alpha, beta, False,map,counter)  # Evaluate child
                    if child_value > max_value:  # Update best value and move
                        max_value, best_move = child_value, move
                    alpha = max(alpha, child_value)
                    if alpha >= beta:
                        break  # Beta cut-off
                map[state_key] = best_move
                with StringIO() as dummy_output, redirect_stdout(dummy_output):
                    self.save(map)
                ...
                for child, move,score,allowed in children_and_move:
                    ...
                    if allowed==False:
                        continue
                    child_value, _ = minimax(child,allowed,False,depth - 1, alpha, beta, True,map,counter)  # Evaluate child
                    if child_value < min_value:  # Update best value and move
                        min_value, best_move = child_value, move
                    beta = min(beta, child_value)
                    if alpha >= beta:
                        break  # Alpha cut-off
                #save the initial state and the best move.
        ...
        def clear_retrieval_count(filename="retrieval_count.txt"):
            """
            Clear the retrieval count by resetting the file to 0.
            
            @Param filename: String, the name of the file to clear.
            """
            with open(filename, "w") as file:
                file.write("0")
        def update_retrieval_count(new_retrievals, filename="retrieval_count.txt"):
            """
            Update the retrieval count in the file by adding new retrievals to the existing count.

            @Param new_retrievals: Integer, the new retrievals to add.
            ...
            if os.path.exists(filename):
                with open(filename, "r") as file:
                    try:
                        total_retrievals = int(file.read().strip())
                    except ValueError:
                        total_retrievals = 0

            total_retrievals += new_retrievals

            with open(filename, "w") as file:
                file.write(str(total_retrievals))
        def solve_sudoku(board):
            ...
            return True
        if game_state.occupied_squares1==[] and game_state.current_player==1:
            clear_retrieval_count()
        counter=[0]
        if game_state.occupied_squares2==[] and game_state.current_player==2:
            clear_retrieval_count()
        all_moves = [Move((i, j), value) for i in range(N) for j in range(N)
                    for value in range(1, N + 1) if possible(i, j, value)]
        ...
        state_map =self.load() or {};
        counter=[0]

        score, move = minimax(game_state,True,True, 4, float('-inf'), float('inf'), alg,state_map,counter)
        update_retrieval_count(counter[0])
